...
const techniques = [
  ...
  },
  {
    ...
    required_telemetry: ["dns_logs", "edr_process_events"],
  },
  {
    technique_id: "T1003",
    technique_name: "OS Credential Dumping",
    required_telemetry: ["edr_process_events", "windows_security_events"],
  },
];

function scoreTechnique(requiredTelemetry) {
  const available = requiredTelemetry.filter((item) => telemetry[item]);
  const missing = requiredTelemetry.filter((item) => !telemetry[item]);
  ...
  return { score, status, available, missing };
}

const coverage = techniques.map((technique) => {
  const result = scoreTechnique(technique.required_telemetry);
  ...
  return {
    ...
    reason:
      result.missing.length > 0
        ...
        : "All required telemetry sources are available.",
  };
});

fs.writeFileSync(outputPath, JSON.stringify(coverage, null, 2));

console.log(`Generated coverage report with ${coverage.length} techniques.`);